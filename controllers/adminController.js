...
export const createClinic = async (req, res) => {
  try {
    const { clinicName, address, city, state, pincode, phone, photos } = req.body;

    if (!clinicName) {
      return res.status(400).json({ success: false, message: "clinicName is required" });
    }

    const clinic = await Clinic.create({ clinicName, address, city, state, pincode, phone, photos });
...
export const updateClinic = async (req, res) => {
  try {
    ...
    const clinic = await Clinic.findByIdAndUpdate(
      clinicId,
      { clinicName, address, city, state, pincode, phone, photos },
      { new: true, runValidators: true }
    );
...
// POST /api/admin/clinics/:clinicId/assign-doctor/:doctorId
export const assignDoctorToClinic = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: `${doctor.name} assigned to ${clinic.clinicName}`,
      doctor,
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
...
// DELETE /api/admin/clinics/:clinicId/remove-doctor/:doctorId
export const removeDoctorFromClinic = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: `${doctor.name} removed from clinic` });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
